...
| **Bichet 2022** — tern haptoglobin | No age effect, no selective (dis)appearance | Model 1 has the lowest AIC; no model is lower by more than ~2 | **Qualitative match** (coefficients differ; the archived haptoglobin scale is ambiguous) |
...
| **Sanghvi 2022** — beetle male weight | Apparent late-life weight increase attributed to selective disappearance of lighter males; lifespan effect P < 0.001 | Model 1 shows the same shape (age **−0.604**, age² **+0.113**); lifespan **+0.433** in Model 6; lowest AIC for Model 5 (Model 6 +57, Model 1 +468) | **Qualitative match**, consistent with the paper's interpretation |
...
| **Sanghvi 2025** — fly fecundity | Zero-inflated negative binomial; lowest AIC for Model 4 (ΔAIC for Models 1, 2, 3, 5 = 38.4, 24.1, 28.0, 2.7) | Not reproducible outside R: the zero-inflated fit needs glmmTMB | **Not verified here** |
...
| **McKenna-Ell 2023** — breeding probability and offspring survival | Breeding: age −0.487, age at last observation +0.188, age × bred as a yearling −0.210. Survival: age −0.232, age at last observation +0.089, early-life recruitment +0.869 | Model 2, binomial (glmmTMB), same terms and random effects. An independent Python Laplace refit of this specification reproduces every coefficient and variance to three decimals; `tests/smoke_test.R` compares the app's own glmmTMB fit | **Independent reproduction; app fit to confirm in R** |
