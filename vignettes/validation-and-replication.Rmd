---
title: "Validation and replication"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Validation and replication}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

disappR is checked in three ways:

* **Simulations with known answers.** An independent implementation of the app's models and decomposition was run on
  50 development scenarios (4,720 simulated datasets) and on ten held-out scenarios with expectations fixed beforehand.
  The full report is installed with the package: `system.file("validation", "VALIDATION.md", package = "disappR")`.
* **Published examples.** Each bundled example ships with the settings that match its paper; the comparison with the
  published estimates is below.
* **Held-out real data.** `inst/validation/heldout/` holds a protocol and a runner for published datasets that were not
  used to build the app (`Rscript tests/scripts/heldout_validation.R`).
